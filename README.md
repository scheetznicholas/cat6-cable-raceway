...
| interior | 23.8 × 16.4 mm — 5 Cat6 stacked 3+2 with room, even 7 mm thick-jacket cable |
| cover | one-piece snap-over, 1.6 mm top and legs, 1 mm chamfered edges |
| base | 2.0 mm floor, 12 mm walls, countersunk #6 / M3.5 screw holes every 100 mm (or double-sided tape) |
| snap | 0.75 mm ramped ridges on both parts, 0.6 mm engagement, 0.68 % leg strain during snap-on |

## Files

- `CableRaceway_H2S_PLA.3mf` — Bambu PLA Basic, 0.20 mm, 3 walls, 15 % gyroid, no supports (2 plates)
...
- `check_mechanism.py` — solid-boolean fit checks: cover seats without interference, is retained when lifted,
  has lateral play, starts the snap freely, a 21 × 14 mm cable bundle clears every piece, end cap fits,
...
3. Room corner: screw one stub to each wall with their mitred ends meeting at the corner, then the one-piece
   ...
   the stub ends.
4. Lay the cables in, then press each cover on: it has a lead-in ramp and clicks when seated.  To remove, slip
   a putty knife under a leg at the wall and lever out — the retention faces are flat so it won't pop off on
   its own.
...
- Everything prints support-free in the exported orientation: bases floor-down, covers top-down, corner
  ...
  a 5 mm per-object brim; the run under its flat arm is a 28.8 mm bridge (fine on an H2S).
- Regenerate after changing parameters: `generate_raceway.py` → `check_mechanism.py` (must print ALL PASS) →
  `make_3mf.py`.
- If a printed cover feels too tight/loose, change `ENGAGE` (0.60) or `LAT` (0.15) in the generator and reprint
  one 150 mm base + cover to test; the other pieces inherit the same profile.
